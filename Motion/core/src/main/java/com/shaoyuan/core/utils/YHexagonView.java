...
public class YHexagonView extends View {
    ...
    public YHexagonView(Context context) {
        super(context);
    }

    public YHexagonView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public YHexagonView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < starCount; i++) {//画多少颗星星
            if (rating > i) {
                canvas.drawBitmap(starT, sss * i, 0, mPaint);
            }
            //画亮的星星
            else if (half && ratingH < 40 && ratingH > 5 && rating == i) {
                canvas.drawBitmap(starH, sss * i, 0, mPaint);
            }
            //画半的星星
            else {
                canvas.drawBitmap(starF, sss * i, 0, mPaint);
            }
            //画暗的的星星
        }
    }
    ...
    private void init() {
        ...
        //这两个自行改成自己的图标
        fillStar = this.getResources().getDrawable(R.drawable.ic_hexagon_green);//亮星星的图标
        emptyStar = this.getResources().getDrawable(R.drawable.ic_hexagon_gray);//暗星星的图标
        halfStar = this.getResources().getDrawable(R.drawable.ic_hexagon_gray);//半星星的图标
    }
